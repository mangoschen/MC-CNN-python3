...
parser.add_argument("-s", "--start", type=int, default=0, help="index of first image to do matching,\
                                                                    this is used for parallel matching of different images")
parser.add_argument("-e", "--end", type=int, default=15, help="index of last image to do matching")
...
out_file = "dispMCCNN.pfm"
out_img_file = "dispMCCNN.pgm"
out_left_file = "disp0MCCNN.pfm"
out_left_img_file = "disp0MCCNN.pgm"
out_right_file = "disp1MCCNN.pfm"
out_right_img_file = "disp1MCCNN.pgm"
...
def main():
    ...
    for left_path in tqdm(img_paths):
        ...
        out_left_path = os.path.join(res_dir, out_left_file)
        out_left_img_path = os.path.join(img_dir, out_left_img_file)
        out_right_path = os.path.join(res_dir, out_right_file)
        out_right_img_path = os.path.join(img_dir, out_right_img_file)
        ...
        print("out_path: {}\nout_time_path: {}\nout_img_path: {}".format(out_left_path, out_time_path, out_left_img_path))
        ...
        # interpolation
        disparity_map = interpolation(left_disparity_map, right_disparity_map, ndisp)
        print("{}: disparity interpolated".format(datetime.now()))

        # subpixel enhancement
        disparity_map = subpixel_enhance(disparity_map, left_cost_volume)
        print("{}: subpixel enhanced".format(datetime.now()))

        # refinement
        # 5*5 median filter 
        disparity_map = median_filter(disparity_map, 5, 5)

        # bilateral filter
        disparity_map = bilateral_filter(left_image, disparity_map, 5, 5, 0, args.blur_sigma, args.blur_threshold)
        print("{}: refined".format(datetime.now()))

        # end timer
        endTime = time.time()

        save_cost_volume_flag = True
        if save_cost_volume_flag:
            out_costl_file = "cost0MCCNN.npy"
            out_costR_file = "cost1MCCNN.npy"
            out_costL_path = os.path.join(img_dir, out_costl_file)
            out_costR_path = os.path.join(img_dir, out_costR_file)
            np.save(out_costL_path, left_cost_volume)
            np.save(out_costR_path, right_cost_volume)


        # save disparity, left_disparity and right_disparity
        util.saveDisparity(disparity_map, out_img_path)
        util.writePfm(disparity_map, out_path)
        util.saveDisparity(left_disparity_map, out_left_img_path)
        util.writePfm(left_disparity_map, out_left_path)
        util.saveDisparity(right_disparity_map, out_right_img_path)
        util.writePfm(right_disparity_map, out_right_path)
