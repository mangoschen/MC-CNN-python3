...
def median_filter(left_disparity_map, filter_height, filter_width):
    ...
    for h in range(height):
        for w in range(width):
            patch_hs = max(0, h - int((filter_height-1)/2))
            patch_he = min(height, h + int((filter_height-1)/2) + 1)
            patch_ws = max(0, w - int((filter_width-1)/2))
            patch_we = min(width, w + int((filter_width-1)/2) + 1)
...
def bilateral_filter(left_image, left_disparity_map, filter_height, filter_width, mean, std_dev, blur_threshold):
    ...
    center_h = int((filter_height - 1)/2)
    center_w = int((filter_width - 1)/2)
